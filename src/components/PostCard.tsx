				...
				<div className="w-full flex">
					<div className="p-4 w-full">
						<div className="w-full h-full border-2 border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
							...
							/>
							<div className="p-6">
								<h3 className="tracking-widest text-xs font-medium mb-1 space-x-2">
									{categories.map((item: any, index: any) => (
										<Link
											className="text-gray-400 hover:underline hover:text-gray-500"
											href={`/${locale}/posts/categories/${item.trim()}`}
											key={index}>
											#{item.trim()}
										</Link>
									))}
								...
								<p className="md:h-20 leading-relaxed mb-3 text-gray-600 dark:text-gray-300 line-clamp-3">
									{description}
								</p>
								<div className="flex items-center flex-wrap justify-between">
									<Link
										href={`/${locale}${
											url === "/posts/about" ? "/about" : url
										}`}
